...
//Rare exceptions to indicate a bug in the _classroomplus code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_classroomplus = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_classroomplus]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.classroomplus.module"
