...
//Rare exceptions to indicate a bug in the _classroom code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_classroom = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_classroom]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.classroom.module"
